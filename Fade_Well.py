#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Fri Nov 27 02:57:57 2020

@author: fadewell
"""

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Mon Nov 23 03:41:55 2020

@author: fadewell
"""

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Fri Nov 20 01:29:11 2020

@author: fadewell
"""

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Sun Sep 20 02:12:01 2020

@author: fadewell
"""

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Mon Sep 14 20:13:02 2020

@author: fadewell
"""

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Sun Sep 13 07:56:08 2020

@author: fadewell
"""

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Sun Sep 13 02:56:35 2020

@author: fadewell
"""

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Sat Sep 12 23:58:12 2020

@author: fadewell
"""

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Sun Sep  6 14:49:58 2020

@author: fadewell
"""

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Sat Sep  5 22:40:09 2020

@author: fadewell
"""

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Sat Sep  5 18:05:00 2020

@author: fadewell
"""

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Fri Sep  4 06:48:45 2020

@author: fadewell
"""

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Thu Sep  3 18:01:00 2020

@author: fadewell
"""

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Wed Sep  2 19:23:50 2020

@author: fadewell
"""

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Tue Sep  1 03:53:09 2020

@author: fadewell
"""

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Mon Aug 31 21:43:19 2020

@author: fadewell
"""

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Sat Aug 29 22:03:31 2020

@author: fadewell
"""

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Sat Aug 29 17:12:42 2020

@author: fadewell
"""

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Thu Aug 27 05:50:58 2020

@author: fadewell
"""

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Wed Aug 26 18:52:12 2020

@author: fadewell
"""

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Tue Aug 25 21:58:01 2020

@author: fadewell
"""

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Tue Aug 25 17:36:26 2020

@author: fadewell
"""

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Sun Aug 23 15:12:14 2020

@author: fadewell
"""

#import math
import qrcode
#import pyqrcode
#import pandas as pd
#import numpy as np
#from pandas import D
import matplotlib.pyplot as plt
import cv2 as cv2
detector = cv2.QRCodeDetector()
#import numpy as np
#JiK = 6
#KiJ = np.zeros(shape=(JiK,JiK))
#for i in range(JiK):
  #  for j in range(JiK):
    #    print (i+1,j+1)
    #    inp = input()
    #    KiJ[i][j] = int(inp)
#import pandas as pd
#import
ensemble = 0
ijk = 0
KiJ=JiK = 0
q1=q2=q3=q4=0
qx1=qx2=qx3=qx4=0
c1 = c2 = c3 = c4 = cx1 = cx2 = cy1 = cy2 = c11 = c22 = c33= c44 = c55 =c66 =c77 =c88 =c99 = ix = iy = iz = iw = cx11 = cx22= cy11=cy22= xy= zy =wx = wy =0
c = d = 0
l = ln= 0
fade = well = gr = ace = 3
fadewell = Grace = 8
fwgd = 1
dimensions = 0
Fs = []
while ensemble <1 :
    white = '-2'   ######## white <---->Green, Cream           1
    Yellow = '-5'   ####-2,-3,-4,----> Yellow==k @ k = 0 ======> 4 White
    Blue = '-7'      ##### -3,-4,-5 ---->Blue == j @ j = -1
    Somon = '5'    ##### 1,3,4,5 -----> Somon = k @ k = 0
    Pink = '-6'      ###### 2,3,4 ----> Pink = ij @ ij = 0
    Green= '-4'      ##### 1,-1,-2,-3 ----> Green = j @ j = 2
    black = '-5' ##### -2,-3,-4 -----> black == i == ij@ ij = 0
    Tree = '2'  #### i 3x
    Cream = '2'  ######## 3,4,5-------> is white too 2
    
    print('How many entities?')
    b = input()
    b = int(b)
    mines = b
    count =-1
    print('what value of entity?')
    a = input()
    a = int(a)
    er = a
    print('How what x?')
    cordx = input()
    cordx = int(cordx)
    print('How what y?')
    cordy = input()
    cordy = int(cordy)
    mn = ()
    mn = (int(cordx),int(cordy))
    Fs.append(mn)
    OfY = OfX = 0
    while dimensions < mines:
       
      #  Somon = str(-int(Green))
    #    Blue = str( int(Green)+1 )     ##### -3,-4,-5 ---->Blue == j @ j = -1
        qqrrr22 = qrcode.make(Cream)
        qqrrr22.save('r000partEnQtities000r.png')
        iiim0g0mgg111 = cv2.imread("r000partEnQtities000r.png")
        gggrreeeeyy0 = cv2.cvtColor(iiim0g0mgg111, cv2.COLOR_XYZ2RGB)
        cream, bbox, straight_qrcode = detector.detectAndDecode(iiim0g0mgg111)

        Cream = int(cream)
        d = 0
        while Cream > 0:
            
            i = 0
            ikj = i
            qr2 = qrcode.make(Tree)
            qr2.save('partEntities00.png')
            img111 = cv2.imread("partEntities00.png")
            greyyyy = cv2.cvtColor(img111, cv2.COLOR_BGR2RGB)
            tree, bbox, straight_qrcode = detector.detectAndDecode(img111)
            Tree = int(Tree)   
            jik = int(Tree)           
            Mines = int(Tree)       
            bb = Mines
            while i < Tree:

                j = 2
                
               
                
                qqrrr22 = qrcode.make(black)
                qqrrr22.save('r000partEntities000r.png')
                iiim00mgg111 = cv2.imread("r000partEntities000r.png")
                ggrreeeeyy0 = cv2.cvtColor(iiim00mgg111, cv2.COLOR_HSV2RGB)
                Black, bbox, straight_qrcode = detector.detectAndDecode(iiim00mgg111)
                i = 3
                if OfX != 0 and OfY != 0:
                    i = 0
                ij = 0
                
                while i < j :
               #     i = ij
                   
                    kik = 1
                    if OfX != 0 and OfY != 0 and c == 'M':
                        kik = 1
                        img = cv2.imread("KiJ20.png")
                        detector = cv2.QRCodeDetector()
                        data, bbox, straight_qrcode = detector.detectAndDecode(img)
                        # if there is a QR code
                        # if there is a QR code
                        if bbox is not None:
                            print(f"QRCode data:\n{data}")
                            # display the image with lines
                            # length of bounding box
                            n_lines = len(bbox)
                            for i in range(n_lines):
                                # draw all lines
                                point1 = tuple(bbox[i][0])
                                point2 = tuple(bbox[(i+1) % n_lines][0])
                                cv2.line(img, point1, point2, color=(255, 0, 0), thickness=2)
                                KiJ =f"{data}"
                                if KiJ =='0':
                                    JiK = KiJ.replace("'","")
                                    gr = int(JiK)
                        img = cv2.imread("KiJ10.png")
                        detector = cv2.QRCodeDetector()
                        data, bbox, straight_qrcode = detector.detectAndDecode(img)
                        if bbox is not None:
                            print(f"QRCode data:\n{data}")
                            # display the image with lines
                            # length of bounding box
                            n_lines = len(bbox)
                            for i in range(n_lines):
                                # draw all lines
                                point1 = tuple(bbox[i][0])
                                point2 = tuple(bbox[(i+1) % n_lines][0])
                                cv2.line(img, point1, point2, color=(255, 0, 0), thickness=2)
                                KiJ =f"{data}"
                                if KiJ =='0':
                                                                   
                                    JiK = KiJ.replace("'","")
                                    ace = int(JiK)
                            img = cv2.imread("KiJ02.png")
                            detector = cv2.QRCodeDetector()
                            data, bbox, straight_qrcode = detector.detectAndDecode(img)
                            # if there is a QR code
                            if bbox is not None:
                                print(f"QRCode data:\n{data}")
                                # display the image with lines
                                # length of bounding box
                                n_lines = len(bbox)
                                for i in range(n_lines):
                                    # draw all lines
                                    point1 = tuple(bbox[i][0])
                                    point2 = tuple(bbox[(i+1) % n_lines][0])
                                    cv2.line(img, point1, point2, color=(255, 0, 0), thickness=2)
                                    KiJ =f"{data}"
                                    if KiJ =='0':
                                        JiK = KiJ.replace("'","")
                                        well = int(JiK)
                            img = cv2.imread("KiJ01.png")
                            detector = cv2.QRCodeDetector()
                            data, bbox, straight_qrcode = detector.detectAndDecode(img)
                            if bbox is not None:
                                print(f"QRCode data:\n{data}")
                                # display the image with lines
                                # length of bounding box
                                n_lines = len(bbox)
                                for i in range(n_lines):
                                    # draw all lines
                                    point1 = tuple(bbox[i][0])
                                    point2 = tuple(bbox[(i+1) % n_lines][0])
                                    cv2.line(img, point1, point2, color=(255, 0, 0), thickness=2)
                                    KiJ =f"{data}"
                                    if KiJ =='0':
                                        JiK = KiJ.replace("'","")
                                        fade = int(JiK) 
                    for ll in range(kik):
                        fadewell = ll+1
                        fwgd = ll
                        qqrr22 = qrcode.make(Green)
                        qqrr22.save('000partEntities000.png')
                        iiim0mgg111 = cv2.imread("000partEntities000.png")
                        ggrreeeyy0 = cv2.cvtColor(iiim0mgg111, cv2.COLOR_RGB2XYZ)
                        green, bbox, straight_qrcode = detector.detectAndDecode(iiim0mgg111)
                    
                        Cream = int(Cream)
                        j = 2
                        kj = j
                        k = 0
                        ijk = ij
                        ji = int(Green)
                        Green = int(Green)
                        x = 0
                        kik = 0
                        cg = 2
                        if ll == 0:
                            img = cv2.imread("KiJ1.png")
                            detector = cv2.QRCodeDetector()
                            data, bbox, straight_qrcode = detector.detectAndDecode(img)
                            # if there is a QR code
                            if bbox is not None:
                                print(f"QRCode data:\n{data}")
                                # display the image with lines
                                # length of bounding box
                                n_lines = len(bbox)
                                for i in range(n_lines):
                                    # draw all lines
                                    point1 = tuple(bbox[i][0])
                                    point2 = tuple(bbox[(i+1) % n_lines][0])
                                    cv2.line(img, point1, point2, color=(255, 0, 0), thickness=2)
                                KiJ =f"{data}"
                                if KiJ =='0':
                                    JiK = KiJ.replace("'","")
                                    l = int(JiK)
                                elif KiJ!= '0':
                                    img = cv2.imread("KiJ2.png")
                                    detector = cv2.QRCodeDetector()
                                    data, bbox, straight_qrcode = detector.detectAndDecode(img)
                            # if there is a QR code
                                    if bbox is not None:
                                        print(f"QRCode data:\n{data}")
                                # display the image with lines
                                # length of bounding box
                                        n_lines = len(bbox)
                                        for i in range(n_lines):
                                    # draw all lines
                                            point1 = tuple(bbox[i][0])
                                            point2 = tuple(bbox[(i+1) % n_lines][0])
                                            cv2.line(img, point1, point2, color=(255, 0, 0), thickness=2)
                                            KiJ =f"{data}"
                                            if KiJ =='0':
                                                JiK = KiJ.replace("'","")
                                                l = int(JiK)
                                            elif KiJ!= '0':
                                                img = cv2.imread("KiJ3.png")
                                                detector = cv2.QRCodeDetector()
                                                data, bbox, straight_qrcode = detector.detectAndDecode(img)
                            # if there is a QR code
                                                if bbox is not None:
                                                    print(f"QRCode data:\n{data}")
                                # display the image with lines
                                # length of bounding box
                                                    n_lines = len(bbox)
                                                    for i in range(n_lines):
                                    # draw all lines
                                                        point1 = tuple(bbox[i][0])
                                                        point2 = tuple(bbox[(i+1) % n_lines][0])
                                                        cv2.line(img, point1, point2, color=(255, 0, 0), thickness=2)
                                                        KiJ =f"{data}"
                                                        if KiJ =='0':
                                                            JiK = KiJ.replace("'","")
                                                            l = int(JiK)
                                                        elif KiJ!= '0':
                                                            img = cv2.imread("KiJ4.png")
                                                            detector = cv2.QRCodeDetector()
                                                            data, bbox, straight_qrcode = detector.detectAndDecode(img)
                            # if there is a QR code
                                                            if bbox is not None:
                                                                print(f"QRCode data:\n{data}")
                                # display the image with lines
                                # length of bounding box
                                                                n_lines = len(bbox)
                                                                for i in range(n_lines):
                                    # draw all lines
                                                                    point1 = tuple(bbox[i][0])
                                                                    point2 = tuple(bbox[(i+1) % n_lines][0])
                                                                    cv2.line(img, point1, point2, color=(255, 0, 0), thickness=2)
                                                                    KiJ =f"{data}"
                                                                    if KiJ =='0':
                                                                        JiK = KiJ.replace("'","")
                                                                        l = int(JiK)
                                                                    elif KiJ != '0':
                                                                        print('clear Grace')
                                ix = int(cordx)+2
                                iy = int(cordy)+2
                                iz = int(cordx)+2
                                iw =  int(cordy)
                                xy = int(cordx) 
                                zy =  int(cordy) 
                                wx = int(cordx)
                                wy=  int(cordy)+2
                                c22 = int(cordx)+1#
                                c33 =  int(cordy)+1+1
                                c44 = int(cordx)+1#
                                c55 =  int(cordy)
                                c66 = int(cordx)+2
                                c77 =  int(cordy)+1##
                                c88 = int(cordx) 
                                c99 =  int(cordy)+1##
                                while j > Green :## i<j
                                   # ij = ijk
                                    #i = ij
                                    qr22 = qrcode.make(Pink)
                                    qr22.save('00partEntities00.png')
                                    iiimmgg111 = cv2.imread("00partEntities00.png")
                                    ggrreeeyy = cv2.cvtColor(iiimmgg111, cv2.COLOR_BGR2HLS)
                                    pink, bbox, straight_qrcode = detector.detectAndDecode(iiimmgg111)
                                    
                                    jk = 2
                                    ###i = 0 or 
                                    ij = 0
                                    ik = k
                                   # JiK = 0
                                    
                                    y = 0
                                    x += 1
                                    cp = 2
                                    while ij < jk:
                                        y+=1
                                       # b = input()
                                        
                                       
                                        k = 0
                                        ik = k
                                        ki = k
                                        qr22 = qrcode.make(Somon)
                                        qr22.save('0partEntities0.png')
                                        iiimg111 = cv2.imread("0partEntities0.png")
                                        ggreeyy = cv2.cvtColor(iiimg111, cv2.COLOR_BGR2Luv)
                                        somon, bbox, straight_qrcode = detector.detectAndDecode(iiimg111)
                                        i = int(Somon)
                                        
                                        Somon = int(Somon)
                                        z = 0
                                        cs = 4
                                        while  k  < Somon :###### X KiJ X
                                            z+=1
                                            
                                             ### -2
                                            qqr2 = qrcode.make(Blue)
                                            qqr2.save('0partEntities.png')
                                            iimg11 = cv2.imread("0partEntities.png")
                                            ggreyy = cv2.cvtColor(iimg11, cv2.COLOR_BGR2LAB)
                                            blue, bbox, straight_qrcode = detector.detectAndDecode(iimg11)
                                            w = 0
                                          #  j = 5
                                            if OfX == x and OfY == y:
                                                j = -1
                                            while j < k:
                                                Nn=0  
                                                w +=1
                                                k = 4
                                                if OfX == x:
                                                    k=-1
                                                qr1 = qrcode.make(Yellow)
                                                qr1.save('partEntities.png')
                                                img11 = cv2.imread("partEntities.png")
                                                greyy = cv2.cvtColor(img11, cv2.COLOR_BGR2HSV)
                                                yellow, bbox, straight_qrcode = detector.detectAndDecode(img11)
                                                
                                              #  Yellow = yellow
                                                while k <  i:
                                                    
                                                    qr = qrcode.make(white)
                                                    qr.save('partEntities0.png')
                                                    img1 = cv2.imread("partEntities0.png")
                                                    greyyy = cv2.cvtColor(img1, cv2.COLOR_BGR2GRAY)
                                                    White, bbox, straight_qrcode = detector.detectAndDecode(img1)
                                                    ik = 0
                                                    Nn+=1
                                                    white = int(white) ### white == jk , -6
                                                    jk = 2
                                                    
                                                    while ik < jk:
                                                        
                                                        white = int(white)
                                                        if ik != white:
                                                            white+= 1
                                                            n = 'c'+str(Nn)
                                                            n = n.replace("'","")
                                                            qr6 = qrcode.make(n)
                                                            qr6.save('White.png')
                                                            aWhite = cv2.imread("White.png")
                                                            cWhite = cv2.cvtColor(aWhite, cv2.COLOR_BGR2RGB)
                                                            data, bbox, straight_qrcode = detector.detectAndDecode(aWhite)
            
                                                    
                                                        if  ik == white:
                                                            white = White
                                                            ik = 3
                                                            
                                                            if (OfX == x and OfY == y and OfX == z and OfY == w and ((a == 'M' or c == 'M')  and  b !='¬H' and d!='M') and ln == 'M' and l == 0):#and c == '¬E') ):
                                                             #   print('Somon',Somon)
                                                               # Grace = 1
                                                                if Nn == 1  and c1 == 'M' and Grace != 1 :
                                                                    print(x+1,'x',y+1,'y')
                                                                    #c11 = 0
                                                                    print('Nn=', Nn)
                                                                    print(x+1,'ix',y+1,'iy')
                                                              #      FS=(x+1,y+1)
                                                                    plt.imshow(cWhite)
                                                                    plt.show()
                                                         
                                                                if Nn==2  and  c2 == 'M' and Grace != 2 :
                                                                  #  print(x+1,'x',y-1,'y')
                                                           #         c11 = 0
                                                                    print('Nn=', Nn)
                                                                    print(x+1,'ix',y-1,'iy')
                                                                    FS=(x+1,y-1)
                                                                    plt.imshow(cWhite)
                                                                    plt.show()
                                                      
                                                                elif Nn==3  and c3 == 'M' and Grace != 3 :
                                                                  #  print(x-1,'x',y-1,'y')
                                                                    print('Nn=', Nn)
                                                                    print(x-1,'ix',y-1,'iy')
                                                                    FS=(x-1,y-1)
                                                                    plt.imshow(cWhite)
                                                                    plt.show()
                                                                  
                                                                elif Nn==4  and   c4 == 'M' and Grace != 4  :
                                                                  #  print(x-1,'x',y+1,'y')   
                                                                    print('Nn=', Nn)
                                                                    print(x-1,'ix',y+1,'iy')
                                                                    FS=(x-1,y+1)
                                                                    plt.imshow(cWhite)
                                                                    plt.show()
                                                               # ln = 1
                                                                
                                                                
                                                                
                                       
                                                    Yellow = int(Yellow)
                                                #    ln = 1
                                                    if  k != Yellow:
                                                        Yellow +=1
                                                        col = [x,w]
                                                        qr6 = qrcode.make(col)
                                                        qr6.save('Yellow.png')
                                                        aYellow = cv2.imread("Yellow.png")
                                                        cYellow = cv2.cvtColor(aYellow, cv2.COLOR_BGR2GRAY)
                                                        data, bbox, straight_qrcode = detector.detectAndDecode(aYellow)
              
                                                    if k == Yellow or c != 'M' and l == 0:                                               
            
                                                        Yellow = yellow
                                                  #      print(Yellow)
                                                        k = int(somon)
                                                        if (OfX == z and OfY == y and OfX == x) and OfY == w+1 and ((a == 'M' or c == 'M')   and b !='¬H' and d!='M' and ln == 'M' and gr==0)  :
                                                   
                                                            FS = (x,w)
                                                            print(c22,'x',c33,'y',Somon,'Somon')
                                                            plt.imshow(cYellow)
                                                            plt.show()
                                                        #    qx3 = '¬E'
                                                        if (OfX == z and OfY == y and OfX == x) and OfY == w-1 and ((a == 'M' or c == 'M')  and b !='¬H' and d!='M' and ln == 'M' and ace ==0)  :
                                                            print(c22,'x',c55,'y',Somon,'Somon')
                                                            FS = (x,w)
                                                            plt.imshow(cYellow)
                                                            plt.show()
                                                
                                                Blue = int(Blue)
                                 
                                                if j != Blue:  #### blue -2
                                                   # j -= 1
                                                    row =[z,y]
                                                    qr6 = qrcode.make(row)
                                                    qr6.save('Blue.png')
                                                    aBlue = cv2.imread("Blue.png")
                                                    cBlue = cv2.cvtColor(aBlue, cv2.COLOR_BGR2HSV)
                                                    data, bbox, straight_qrcode = detector.detectAndDecode(aBlue)
                                                    Blue +=1
                                               #  
                                                   
                                                if j == Blue or c != 'M' and l == 0:
                                                    Blue = blue
                                                    
                                                    j = int(Cream)
                                                    jk = j
                                                    
                                                    if ( OfX == x and OfY == y ) and OfX == z+1  and ((a == 'M' or c == 'M') and b !='¬H' and d!='M' and well == 0 and cy1 =='M' and ln =='M'):# and b != '¬E':
                                              
                                                        print(c66,'x',c77,'y',Somon,'Somon')#
                                                        FS = (z,y)
                                                        plt.imshow(cBlue)
                                                        plt.show()
                                                    elif ( OfX == x and OfY == y ) and  OfX == z-1 and ((a == 'M' or c == 'M')  and b !='¬H' and d!='M' and fade == 0 and cy2=='M' and ln == 'M' ):# and b != '¬E':
                                                        print(c88,'x',c77,'y',Somon,'Somon')
                                                        plt.imshow(cBlue)
                                                        plt.show()
                                                    #    Grace = fadewell
                                                     #   fadewell = 1
                                             
                                                       
                                                        
                                                
                                                
                                                k = ki
            
                                            if k != i :
                                               i -= 1
                                               if  b != '¬H' and c == 'M' and d!='M':
                                                       a = 'M'
                                                       black = -1
                                              
                                               
                                               
                                                       
                                                     
                                            elif k == i :
                                                
                                                if ( OfX == x and OfY == y and (a != '¬M' and (a!='M' or b != '¬H')) and c == 'M' and l == 0) :
                                                   FS=[x,y]
                                                  
                                                   
                                                   if Somon == 5 and b != '¬H' :
                                                       
                                                       
                                                       fadewell = l
                                                       ln = 1
                                                       if c1 == 'M':
                                                           Grace = 1
                                                           ln = 'M'
                                                           print(x+1,'x',y+1,'y')   
                                                           plt.imshow(ggreyy)
                                                           plt.show()
                                                  #         Grace = fadewell
                                                   elif Somon==4 and b != '¬H'  :
                                                       Grace = 2
                                                       ln = 1 
                                                     #  fadewell = l
                                                       if c2 == 'M':
                                                           ln = 'M'
                                                           print(x+1,'x',y-1,'y')   
                                                           plt.imshow(ggreyy)
                                                           plt.show()
                                                          # Grace = fadewell
                                                   elif Somon==3 and b != '¬H' :
                                                       ln = 1
                                                       Grace = 3
                                                       fadewell = l
                                                       if c3 == 'M':
                                                           ln = 'M'
                                                           print(x-1,'x',y-1,'y')   
                                                           plt.imshow(ggreyy)
                                                           plt.show()
                                                       #    Grace = fadewell
                                                   elif Somon==2 and b != '¬H' :
                                                       Grace = 4
                                                       ln = 1
                                                       if c4 == 'M':
                                                           ln = 'M'
                                                           print(x-1,'x',y+1,'y')   
                                                           plt.imshow(ggreyy)
                                                           plt.show()
                                                        #   Grace = fadewell
                                                   elif Somon == 1 and b != '¬H':
                                                       
                                                       print(x,'x',y,'y')   
                                                      
                                                     
                                                       
                                                       Somon = k
                                                Somon -=1
                                                if Somon!= i:
                                                 
                                                   i = int(somon)
                                                   z = 0
                                            
                                        Somon = somon
                                       
                                        
                                       
                                        
                                        
                                    
                                        i = ij   
                                        Pink = int(Pink)
                                        
                                        if ij != Pink:
                                           Pink+=1
                                           qr6 = qrcode.make(OfY)
                                           qr6.save('Pink.png')
                                           aPink= cv2.imread("Pink.png")
                                           cPink = cv2.cvtColor(aPink, cv2.COLOR_BGR2Luv)
                                           data, bbox, straight_qrcode = detector.detectAndDecode(aPink)
                                           
                                        #   print('False')
                                           
                                           
                                           if ( OfX == x and  OfY == y and OfY != 0 and OfX!=0)  and b !='¬H'  and c == 'M' and d!='M' and l == 0 and cx1 == 'M':
                                              print(x,'x',y+1,'y') ###=='
                                              
                                              plt.imshow(cPink)
                                              plt.show()
                                          
                                           if ( OfX == x and  OfY == y+1 and OfY != 0 and OfX!=0)   and b !='¬H'  and c == 'M' and d!='M' and l == 0 and cx2 =='M':
                                              print(x,'x',y,'y') 
                                              
                                              plt.imshow(cPink)
                                              plt.show()
                                            
                                        
                                        if ij == Pink or c != 'M': 
                                      #      print('here')
                                            ij = 101
                                            
                                        
                                        
                                        
                                               
                             
                                    Pink = pink
                                    
                              
                                    
                                    if ji!=j:
                                        
                                        qr6 = qrcode.make(OfX)
                                        qr6.save('Green.png')
                                        aGreen = cv2.imread("Green.png")
                                        cGreen = cv2.cvtColor(aGreen, cv2.COLOR_BGR2HLS)
                                        data, bbox, straight_qrcode = detector.detectAndDecode(aGreen)
                                        if OfX == x and OfY != 0 and OfX!=0  and b !='¬H' and c == 'M' and d!='M' and l == 0 and cy1 == 'M':
                                            print(x+1,'x',OfY,'y')
                                            
                                            # if there is a QR code
                                            
                                            plt.imshow(cGreen)
                                            plt.show()
                                          
                                    
                                        if  OfX == x+1 and OfY != 0 and OfX!=0 and b !='¬H'  and c == 'M' and d!='M' and l == 0 and cy2 == 'M':
                                            print(x,'x',OfY,'y')
                                            
                                            plt.imshow(cGreen)
                                            plt.show()
                                            
                                                  
                                        Green = int(Green)
                                        Green +=1
                                    
                            
                                    if ji==j :
                                        j = kj
                                        j -=1
                                        kj = j
                                       # 
                                     #   
                                    
                                    ij = ijk
                                   # i = int(black)
                                #j = ij
                                Green = int(green)
             
                  
                        black = int(black)  
                        print('loading a',a)
                    if ij != black:####black
                        
                        if  OfX != 0 and OfY != 0 :
                            
                            if black == -5 and a != 'M' :
                                KiJ ='c'+str(6+ black)
                                KiJ = KiJ.replace("''","")
                                qr6 = qrcode.make(KiJ)
                                qr6.save('KiJ1.png')
                                anWhite = cv2.imread("KiJ1.png")
                                ccWhite = cv2.cvtColor(anWhite, cv2.COLOR_RGB2XYZ)
                                data, bbox, straight_qrcode = detector.detectAndDecode(anWhite)
                            
                            
                                print(KiJ)
                                print(OfX+1,'x',OfY+1,'y')
                                plt.imshow(ccWhite)
                                plt.show() 
                            #    print (n)
                                print(' Enter Value, M , ¬M')
                                c1 = input ()  
                                
                                if c1 == 'M':
                                    qr6 = qrcode.make(0)
                                    qr6.save('KiJ1.png')
                                    anWhite = cv2.imread("KiJ1.png")
                                    ccWhite = cv2.cvtColor(anWhite, cv2.COLOR_RGB2XYZ)
                                    data, bbox, straight_qrcode = detector.detectAndDecode(anWhite)
                                   
                            elif black==-4 and  a != 'M' :
                                KiJ ='c'+str(6+ black)
                                KiJ = KiJ.replace("''","")
                                qr6 = qrcode.make(KiJ)
                                qr6.save('KiJ2.png')
                                anWhite = cv2.imread("KiJ2.png")
                                ccWhite = cv2.cvtColor(anWhite, cv2.COLOR_RGB2XYZ)
                                data, bbox, straight_qrcode = detector.detectAndDecode(anWhite)
                                print(OfX+1,'x',OfY-1,'y')
                                plt.imshow(ccWhite)
                                plt.show() 
                                print(' Enter Value, M , ¬M')
                                c2 = input ()  
                                if c2 == 'M':
                                    qr6 = qrcode.make(0)
                                    qr6.save('KiJ2.png')
                                    anWhite = cv2.imread("KiJ2.png")
                                    ccWhite = cv2.cvtColor(anWhite, cv2.COLOR_RGB2XYZ)
                                    data, bbox, straight_qrcode = detector.detectAndDecode(anWhite)
                                   
                            elif black==-3 and a != 'M'  :
                                KiJ ='c'+str(6+ black)
                                KiJ = KiJ.replace("''","")
                                qr6 = qrcode.make(KiJ)
                                qr6.save('KiJ3.png')
                                anWhite = cv2.imread("KiJ3.png")
                                ccWhite = cv2.cvtColor(anWhite, cv2.COLOR_RGB2XYZ)
                                data, bbox, straight_qrcode = detector.detectAndDecode(anWhite)
                                print(OfX-1,'x',OfY-1,'y')
                                plt.imshow(ccWhite)
                                plt.show() 
                                print(' Enter Value, M , ¬M')
                                c3 = input ()  
                                if c3 == 'M':
                                    qr6 = qrcode.make(c3)
                                    qr6.save('KiJ3.png')
                                    anWhite = cv2.imread("KiJ3.png")
                                    ccWhite = cv2.cvtColor(anWhite, cv2.COLOR_RGB2XYZ)
                                    data, bbox, straight_qrcode = detector.detectAndDecode(anWhite)
                                    
                            elif black==-2 and  a != 'M'  :
                                KiJ ='c'+str(6+ black)
                                KiJ = KiJ.replace("''","")
                                qr6 = qrcode.make(KiJ)
                                qr6.save('KiJ4.png')
                                anWhite = cv2.imread("KiJ4.png")
                                ccWhite = cv2.cvtColor(anWhite, cv2.COLOR_RGB2XYZ)
                                data, bbox, straight_qrcode = detector.detectAndDecode(anWhite)
                                print(OfX-1,'x',OfY+1,'y')   
                                plt.imshow(ccWhite)
                                plt.show() 
                                print(' Enter Value, M , ¬M')
                                c4 = input ()  
                                if c4 == 'M':
                                    qr6 = qrcode.make(0)
                                    qr6.save('KiJ4.png')
                                    anWhite = cv2.imread("KiJ4.png")
                                    ccWhite = cv2.cvtColor(anWhite, cv2.COLOR_RGB2XYZ)
                                    data, bbox, straight_qrcode = detector.detectAndDecode(anWhite)
                                    
                            elif black == -1 :
                                b = 'H'
                                if a == 'M':
                                    d = '¬M'
                                    OfX = OfY = 0
                                    a = er
                                    c = '¬M'
                                  
                                    
                        black +=1
                        
           
                            
                    
                    if black == ij or b == '¬H':
                        ij= 101
                        i = ij
                       
                        black = Black
                        
   
                
             #   print(i,'i',ij,'ij',ijk,'ijk')
             #   
                  
               # Tree -= 1
                if jik != i:
                    i = ikj     
                    i += 1
                    ikj = i
                 #   print('Entities',i)
                   # Mines -= 1
                    if OfX != 0 and OfY == 0:
                        plt.imshow(ggrreeeeyy0)
                        plt.show() 
                        KiJ ='cx'+str('2')
                        KiJ = KiJ.replace("''","")
                        qr6 = qrcode.make(KiJ)
                        qr6.save('KiJ10.png')
                        anWhite = cv2.imread("KiJ10.png")
                        ccWhite = cv2.cvtColor(anWhite, cv2.COLOR_RGB2XYZ)
                        data, bbox, straight_qrcode = detector.detectAndDecode(anWhite)
                 #       print('which y+1,y-1 has ')
               #        OfY = input()
                        OfY = cordy
                        print(OfX,'x', OfY-1,'y')
                        print(' Enter Value, M , ¬M')
                        cx2 = input()
                        
                        if cx2 == 'M':
                            qr6 = qrcode.make(0)
                            qr6.save('KiJ10.png')
                            anWhite = cv2.imread("KiJ10.png")
                            ccWhite = cv2.cvtColor(anWhite, cv2.COLOR_RGB2XYZ)
                            data, bbox, straight_qrcode = detector.detectAndDecode(anWhite)
                          # count+=1
                    elif OfX != 0 and OfY != 0 and b !='¬H':
                        KiJ ='cx'+str('1')
                        KiJ = KiJ.replace("''","")
                        qr6 = qrcode.make(KiJ)
                        qr6.save('KiJ20.png')
                        anWhite = cv2.imread("KiJ20.png")
                        ccWhite = cv2.cvtColor(anWhite, cv2.COLOR_RGB2XYZ)
                        data, bbox, straight_qrcode = detector.detectAndDecode(anWhite)
                        plt.imshow(ggrreeeeyy0)
                        plt.show() 
                        print(OfX,'x', OfY+1,'y')
                        print(' Enter Value, M , ¬M')
                        cx1 = input()
                        if cx1 == 'M':
                            qr6 = qrcode.make(0)
                            qr6.save('KiJ20.png')
                            anWhite = cv2.imread("KiJ20.png")
                            ccWhite = cv2.cvtColor(anWhite, cv2.COLOR_RGB2XYZ)
                            data, bbox, straight_qrcode = detector.detectAndDecode(anWhite)
                    

                if jik ==i:
                    i = int(Tree)
                    
                   # Tree-=1
            Tree = tree
  

            
            
            if OfX == 0:
                plt.imshow(gggrreeeeyy0)
                plt.show()
                KiJ ='cy'+str('2')
                KiJ = KiJ.replace("''","")
                qr6 = qrcode.make(KiJ)
                qr6.save('KiJ01.png')
                anWhite = cv2.imread("KiJ01.png")
                ccWhite = cv2.cvtColor(anWhite, cv2.COLOR_RGB2XYZ)
                data, bbox, straight_qrcode = detector.detectAndDecode(anWhite)
                OfX = cordx
                print(OfX-1,'x',cordy,'y')
                
                print(' Enter Value, M , ¬M')
                cy2 = input()
                if cy2 == 'M':
                   qr6 = qrcode.make(0)
                   qr6.save('KiJ01.png')
                   anWhite = cv2.imread("KiJ01.png")
                   ccWhite = cv2.cvtColor(anWhite, cv2.COLOR_RGB2XYZ)
                   data, bbox, straight_qrcode = detector.detectAndDecode(anWhite)
             
            elif OfX != 0:
                if b != '¬H':
                    plt.imshow(gggrreeeeyy0)
                    plt.show()
                    KiJ ='cy'+str('1')
                    KiJ = KiJ.replace("''","")
                    qr6 = qrcode.make(KiJ)
                    qr6.save('KiJ02.png')
                    anWhite = cv2.imread("KiJ02.png")
                    ccWhite = cv2.cvtColor(anWhite, cv2.COLOR_RGB2XYZ)
                    data, bbox, straight_qrcode = detector.detectAndDecode(anWhite)
                    print(OfX+1,'x',cordy,'y')
                    print(' Enter Value, M , ¬M')
                    cy1 = input()
                    if cy1 =='M':
                        qr6 = qrcode.make(0)
                        qr6.save('KiJ02.png')
                        anWhite = cv2.imread("KiJ02.png")
                        ccWhite = cv2.cvtColor(anWhite, cv2.COLOR_RGB2XYZ)
                        data, bbox, straight_qrcode = detector.detectAndDecode(anWhite)
                        mines -= er
                    print (mines)
                elif b =='¬H':
                    print('QR Cream QR')
                    c = 0
                    a = er
                    
                    b  = mines
                    OfX = OfY = 0
                    
            Cream -= 1
        Cream = cream
     #   print(' Enter Value, M , ¬M')    
        c = 'M'
        dimensions += 1

    ensemble += 1
